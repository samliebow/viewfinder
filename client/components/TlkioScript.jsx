...
const TlkioScript = ({ codestitch, tlkio, zoom, name, email, startTime, loggedIn }) => {
  const noShowEmailSubject = `Your Hack Reactor Technical Interview Appointment is Currently In Progress: Connect Here`;
  // const noShowEmailBody = `Hi ${name.split(' ')[0]},\n\nIt is currently time for your technical interview and we have been waiting for you.\n\nPlease connect with us in the next five minutes by going to the following URL: ${tlkio}\n\nThanks,\n${loggedIn.split(' ')[0]}`; // While convenient, adding this as a query parameter replaces the entire body and thus deletes e-mail signatures.
  const noShowGmailComposeHyperLink = `\` [\`${email}\`](https://mail.google.com/mail/?view=cm&to=${email}&su=${encodeURIComponent(noShowEmailSubject)}) \``;
  return (
  ...
  \`(Send to ${email ? noShowGmailComposeHyperLink : 'their email'} if they don't show ${startTime ? 'by ' + startTime.clone().add(5, 'minutes').format('h:mm') : 'within 5 minutes'}:)\`
  
  \`Hi ${name.split(' ')[0]},\`

  \`It is currently time for your technical interview and we have been waiting for you.\`
  
  \`Please connect with us in the next five minutes by going to the following URL:\` [\`${tlkio}\`](${tlkio})
  
  \`Thanks,\`

  \`${loggedIn ? loggedIn.split(' ')[0] : ''}\`
  `}
